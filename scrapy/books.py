...
def getThumbsUrls(url: str):
    ...
    return {"artcles_urls": artcles_urls, "nextUrl": buttonNext}
...
def getAllBookInfos():
    ...
    for index in range(len(booksUrl)):
        bookInfo = getBookInfo(
            f"https://books.toscrape.com/catalogue/{booksUrl[index]}"
        )
        allBooksInfos.extend(bookInfo)
        status["scraping_data"] = (
            f">>> scraping data: {(index * 100) / len(booksUrl)}%"
        )
        show_status()
    return allBooksInfos
